...
// current operator is assigned by pssFight
let operator;

// paper scissors stone initialize
let pssDone = false;
let pssBtn = document.querySelector('#pssBtn')
let pssCloseBtn = document.querySelector('#pssCloseBtn')
let resolvePss;
// paper scissors stone choosing(clicking) promise
let pssPromise = new Promise(resolve => {
    resolvePss = resolve;
});
let playerPss;

let gestures = document.querySelectorAll('.pss-modal .modal-body button');

for (let gesture of gestures) {
    gesture.addEventListener('click', pssChoose)
}
...
function seeRecords() {
    const records = fetch('/recordAdvanced')
...
async function playing() {
    ...
    do {
        ...
        pssDone = false;
        // wait 0.5 sec to let player review the blocks for a while
        await thinking(500)
    } while (!game_over)
...
async function pssFight() {
    // 0 is paper
    // 1 is scissors
    // 2 is stone
    let computerPss = getRandomInt(3);

    await pssPromise;

    pssPromise = new Promise(resolve => {
        resolvePss = resolve;
    });

    // result
    // win
    if ((playerPss === 0 && computerPss == 2) || (playerPss === 1 && computerPss === 0) || (playerPss === 2 && computerPss === 1)) {
        pssDone = true;
        alert('Win. Your turn!')
        pssCloseBtn.click();
        operator = 0;
    ...
    // lose
    if ((playerPss === 2 && computerPss == 0) || (playerPss === 0 && computerPss === 1) || (playerPss === 1 && computerPss === 2)) {
        pssDone = true;
        alert("Lose. computer's turn!")
        pssCloseBtn.click();
        operator = 1;
    }
    // tie
    if (playerPss === computerPss) {
        pssDone = false;
...
function pssChoose(event) {
    let gestureValue = event.target.value;
    if (gestureValue === 'paper') {
        playerPss = 0;
    }
    if (gestureValue === 'scissors') {
        playerPss = 1;
    }
    if (gestureValue === 'stone') {
        playerPss = 2;
    }
    resolvePss()
    return
...
// for computer to generate his pss choice
